App.factory('GroupsModel', function(SQLiteService, DB_CONFIG) {
    ...
    return {
        ...
        get_group_and_user_from_guid : function(GroupsUserId, callback) {
            var DB = new SQLiteService();
            DB.select("g.*, u.*");
            DB.from("groups as g");
            DB.join("groups_users as gu", "g.GroupId = gu.GroupId AND gu.GroupsUserId = " + GroupsUserId);
            DB.join("users as u", "gu.UserId = u.UserId");
            DB.query(callback);
        },
